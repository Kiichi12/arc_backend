...
def delete_file(request, id):
    ...
    try:
        ...
        return Response({"error": "File does not exist or is unauthorized"}, status=status.HTTP_404_NOT_FOUND)
    file.delete()
...
def rename_file(request, id):
    ...
    return Response({
        ...
        "name": file.name,
        "parent": {
            "id": file.parent.id if file.parent else None,
            "name": file.parent.name if file.parent else None
        }
...
def delete_comment(request, id):
    ...
    if comment.user != request.user and not request.user.role == 'admin':
        return Response({'error': 'Unauthorized'}, status=status.HTTP_403_FORBIDDEN)
...
def get_all_files(request):
    ...
    allowed_sort = {
        ...
        'average_rating': 'average_rating',
        'views': 'user_view_count'
    }
    if sort_by in allowed_sort:
        files = files.order_by(allowed_sort[sort_by])
    elif sort_by:
